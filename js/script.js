...
function renderClasses() {
  let html = apiData.map((item) => {
    return `<div class="class  d-flex flex-column justify-content-between p-2 mt-2" style="background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)),url(${item.image})"> 
    <h3 class="">${item.title}</h3><p class="">${item.type}</p><button class="btn w-50 " style="margin-left: auto">Sign Up</button></div>`;
  });
  $("#classes").append(html);
}
